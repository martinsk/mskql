...
EXPLAIN SELECT o.id, c.name, p.pname, o.qty FROM t3j_orders o JOIN t3j_customers c ON o.customer_id = c.id JOIN t3j_products p ON o.product_id = p.id ORDER BY o.id
-- expected output:
100|alice|widget|3
101|bob|gadget|1
102|alice|gadget|2
Project
  Sort
    Hash Join
      Hash Join
        Seq Scan on t3j_orders
        Seq Scan on t3j_customers
      Seq Scan on t3j_products
