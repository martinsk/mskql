...
-- input:
SELECT r.name, SUM(d.amount) AS pipeline FROM deals d JOIN reps r ON d.rep_id = r.id GROUP BY r.name HAVING SUM(d.amount) > 60000 ORDER BY pipeline DESC, r.name;
-- expected output:
Alice|95000
Bob|95000
Carol|95000
