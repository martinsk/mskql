-- tutorial: recursive CTEs - limit recursion depth (recursive-ctes.html)
-- setup:
...
-- input:
WITH RECURSIVE org AS (SELECT id, name, title, 0 AS depth FROM employees WHERE manager_id IS NULL UNION ALL SELECT e.id, e.name, e.title, o.depth + 1 FROM employees e JOIN org o ON e.manager_id = o.id WHERE o.depth < 2) SELECT name, title, depth FROM org ORDER BY depth, name;
-- expected output:
Alice|CEO|0
