-- tutorial: EXISTS subquery (multi-table-joins.html)
-- setup:
...
-- input:
SELECT c.name FROM customers c WHERE EXISTS (SELECT 1 FROM orders o JOIN order_items oi ON o.id = oi.order_id JOIN products p ON oi.product_id = p.id WHERE o.customer_id = c.id AND p.price * oi.qty > 60) ORDER BY c.name;
-- expected output:
Alice
Bob
Carol
